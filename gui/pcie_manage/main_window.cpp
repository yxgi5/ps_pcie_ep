...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
{
    setWindowTitle(tr("PS-PCIE DMA读写数据测试工具"));
    ...
    pUserUi->pPcieManage->pLineEditAddr->setText("0x100000");
    pUserUi->pPcieManage->pLineEditLength->setText("128");
    ...
    {
        pUserUi->pButStart->setText(tr("无设备！"));
        pUserUi->pPcieManage->hide();
    }
    pUserUi->pPcieManage->pComboBox->setCurrentIndex(0);
	...
	pDataBuff = (char *)aligned_alloc(dataBuffLen, dwPageSize);
    pTimer = new QTimer;
    opt_mode = 0;
...
{
    pUserUi->pButStart->setText(tr("设备故障"));
